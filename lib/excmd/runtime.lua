...
local COMPILED_SCRIPT_CACHE_QUEUE = {}
local COMPILED_SCRIPT_CACHE_HEAD = 1
local COMPILED_SCRIPT_CACHE_SIZE = 0
...
local function resolve_vlua_function(callee)
    ...
    if rest:sub(1, 7) == "require" then
        ...
        if not ok then
            error(Error(5108, tostring(modtbl)))
        end
...
local function runtime_table_kind(tbl)
    if type(tbl) ~= "table" then
        return nil
    end

    local kind = (getmetatable(tbl) or EMPTY_ARGS).__vimxpr_kind
...
local function resolve_path_key(container, seg)
    ...
    local kind = runtime_table_kind(container)
    if kind == "list" then
...
local function assign_lhs(lhs, value, state, error_text)
    ...
    if s:sub(1, 1) == "[" and s:sub(-1) == "]" then
        ...
        for i = 1, #items do
            local item = items[i]
            if item ~= "" then
                local ok = assign_lhs(item, src[i], state, item)
    ...
    local parsed = split_lvalue_base_and_tail(s) or EMPTY_ARGS
    if parsed.tail and parsed.tail ~= "" then
        ...
        if segments and #segments > 0 then
            ...
            if type(slot_tbl) ~= "table" then
                return Error(1203, error_text or lhs)
            end
...
local function unlet(names, _, state)
    local tokens = type(names) == "table" and (names.names or names)
    if tokens then
        for i = 1, #tokens do
...
local function runtime_mod(a, b)
...
local strip = Payload.trim
local lstrip = Payload.lstrip
...
local function enrich_runtime_error(err, rt, opts, state, script, fallback)
    ...
    return Error(5108, with_context_message(base, rt, opts, state, script))
end

local function clear_compiled_script_cache()
    COMPILED_SCRIPT_CACHE = {}
    COMPILED_SCRIPT_CACHE_QUEUE = {}
    COMPILED_SCRIPT_CACHE_HEAD = 1
    COMPILED_SCRIPT_CACHE_SIZE = 0
end
...
local function cache_compiled_script(script, compiled)
    if COMPILED_SCRIPT_CACHE[script] == nil then
        if COMPILED_SCRIPT_CACHE_SIZE == COMPILED_SCRIPT_CACHE_MAX then
            local evict = COMPILED_SCRIPT_CACHE_QUEUE[COMPILED_SCRIPT_CACHE_HEAD]
            COMPILED_SCRIPT_CACHE[evict] = nil
            COMPILED_SCRIPT_CACHE_QUEUE[COMPILED_SCRIPT_CACHE_HEAD] = script
            COMPILED_SCRIPT_CACHE_HEAD = (COMPILED_SCRIPT_CACHE_HEAD % COMPILED_SCRIPT_CACHE_MAX) + 1
        else
            local tail = ((COMPILED_SCRIPT_CACHE_HEAD + COMPILED_SCRIPT_CACHE_SIZE - 1)
                % COMPILED_SCRIPT_CACHE_MAX) + 1
            COMPILED_SCRIPT_CACHE_QUEUE[tail] = script
            COMPILED_SCRIPT_CACHE_SIZE = COMPILED_SCRIPT_CACHE_SIZE + 1
        end
    end
...
local strip_trailing_comment = Payload.strip_trailing_comment
local rejoin_equals = Payload.rejoin_option_assignments
...
function Runtime:get_exec_cursor()
...
function Runtime:push_script_frame()
...
function Runtime:echo_values(cmd, values)
    local parts = {}
    for i = 1, #values do
        parts[#parts + 1] = runtime_string(values[i])
    end
    local line = table.concat(parts, " ")
    if cmd == "echo" then ExMsg.echo(line)
    elseif cmd == "echoerr" then ExMsg.echoerr(line)
    elseif cmd == "echomsg" then ExMsg.echomsg(line)
    else ExMsg.echon(line) end
    return true
end

function Runtime:cmp(a, op, b)
...
function Runtime:truthy(v)
    return truthy(v)
end

function Runtime:assign(lhs, value, error_text)
    local rv = assign_lhs(lhs, value, self.state, error_text)
    if Error.IsError(rv) then error(rv) end
    return rv
end
...
function Runtime:unlet(spec, bang)
    return unlet(spec, bang, self.state)
end

function Runtime:iter(val)
    return iter_list(val)
end

function Runtime:_push_script_ctx()
    local state = self.state
...
function Runtime:return_exc(value)
    return { __ret = true, value = value }
end

function Runtime:break_exc()
    return { __break = true }
end

function Runtime:continue_exc()
...
function Runtime:exec_script(script)
...
function Runtime:exec_user_command_script(def, script)
...
function Runtime:exec_compiled(fn)
    return with_lazyredraw_block(function()
        local fn_ok, fn_rv = with_runtime_script_context(self, function()
            return fn(self.state, self)
        end)
...
function Runtime:execute(expr)
    ...
    local function expand_execute_dollar_single_quoted(input_expr)
        ...
        while i <= n do
            ...
            elseif ch == "{" then
                ...
                if j > n then
                    error(Error(15, "Unterminated interpolation in execute() string"))
                end
    ...
    if is_interp then
        return with_cleared_command_modifiers(self, function()
...
function Runtime:execute_values(values)
...
local COMMAND_MODIFIER_SPECS = {
    vertical = { split_orientation = "vertical", equalize_axis = "vertical" },
    horizontal = { equalize_axis = "horizontal" },
    leftabove = { split_location = "aboveleft" },
    aboveleft = { split_location = "aboveleft" },
    rightbelow = { split_location = "belowright" },
    belowright = { split_location = "belowright" },
    topleft = { split_location = "topleft" },
    botright = { split_location = "botright" },
}
...
local function _build_cmd_context(cursor, win, spec)
    ...
    if spec and type(spec.ws_args) == "table" and #spec.ws_args ~= 0 then
        ctx.args = spec.ws_args
    end
...
local function _map_command_opts(opts, map_meta)
    local out = map_meta and { map_meta = map_meta }
    if opts.buffer then
        out = out or {}
...
local function _strtoseq_tolerant(s)
    ...
    while i <= n do
        ...
        if ch ~= "<" then
            ...
            local ok, part = pcall(Key.strtoseq, s:sub(i, j - 1))
            if not ok then
...
local function _strtoseq_normal_literal(s)
    ...
    local ok, seq = pcall(Key.strtoseq, escaped)
    if not ok then
...
local function _filetype_enable_detection()
    ...
    local ok, err = ScriptSource.source_runtime("filetype.lua")
    if not ok then
        return err
    end
    ok, err = ScriptSource.source_runtime("filetype.vim")
    if not ok then
        return err
    end
    return true
...
local function _filetype_enable_plugin()
    local ok, err = ScriptSource.source_runtime("ftplugin.vim")
    if not ok then
        return err
    end
    return true
...
local function _filetype_disable_plugin()
    local ok, err = ScriptSource.source_runtime("ftplugof.vim")
    if not ok then
        return err
    end
    return true
...
local function _filetype_enable_indent()
    local ok, err = ScriptSource.source_runtime("indent.vim")
    if not ok then
        return err
    end
    return true
...
local function _filetype_disable_indent()
    local ok, err = ScriptSource.source_runtime("indoff.vim")
    if not ok then
        return err
    end
    return true
...
local function _emit_highlight_listing(names)
    ...
    for i = 1, #names do
        ...
        local suffix = Highlight.ListingSuffix(name)
        if suffix ~= "" then
            ExMsg.echon(suffix)
        end
        ExMsg.flush()
...
local function _colors_name_var()
    local name = scopes._g and scopes._g.colors_name
...
local function _colorscheme_search_roots()
    ...
    append(RuntimePath.get_search_list())
    append(RuntimePath.list_packages("opt"))
    return roots
end
...
local function _load_colorscheme(name)
    ...
    if not ok then
        Highlight.CancelPaletteTracking()
        error(err)
    end
...
function Runtime:put(argstr, bang, cmdctx)
...
function Runtime:put_compiled(payload, bang, spec)
    local win = windows[curwin]
...
function Runtime:substitute(argstr, _bang, cmdctx)
...
function Runtime:global(argstr, bang, cmdctx)
    ...
    for i = 1, #matched_lines do
        ...
        if inner == "" then
            ExMsg.echo((buf:get_line(line_no, true) or "") .. "\n")
        else
            local ok, rv = pcall(function()
                return self:exec_script(inner)
            end)
            if not ok then
                self.state.v.errmsg = rv:toString()
            end
        end
...
function Runtime:sort(argstr, bang, cmdctx)
    local raw = strip(argstr)
...
function Runtime:define_autocmd(rest, bang)
...
function Runtime:doautoall(rest)
    local event
    if type(rest) == "table" then
        event = rest.event
    else
...
function Runtime:doautocmd(rest)
...
function Runtime:augroup(raw, bang)
    local arg = strip(raw)
    if arg == "" then
        return true
    end
...
function Runtime:delcommand(raw)
    local args = split_ws(raw)
...
function Runtime:delfunction(raw, bang)
...
function Runtime:comclear()
    Runtime._USER_COMMANDS = {}
    return true
end

function Runtime:filetype(argstr)
    ...
    if #args == 0 then
        ...
        ExMsg.echo("filetype detection:" .. detect .. "  plugin:" .. plugin .. "  indent:" .. indent)
        return true
    end
...
function Runtime:define_command(rest, bang)
    local parsed = type(rest) == "table" and rest or Payload.parse_command_definition(rest)
    local parts = parsed.parts
    local nargs = parsed.nargs
...
function Runtime:invoke_compiled_command(spec)
    spec = spec or {}
    ...
    if not dispatch_cmd then
        if Commands.is_builtin(lname) then
            dispatch_cmd = lname
        else
...
function Runtime:invoke_compiled_builtin_command(dispatch_cmd, spec)
    local win = windows[curwin]
    local cmdctx = _build_cmd_context(self:get_exec_cursor(), win, spec)
...
function Runtime:invoke_command(name, argstr, bang)
...
function Runtime.new(init_state, init_opts)
    ...
    local function _resolve_find_name(raw)
        ...
        local found = Builtins.fn.findfile(target)
    ...
    local function _find_window_for_path(target_abs)
        for _, win in pairs(windows) do
            ...
            if type(name) == "string" and name ~= "" then
                local ok, buf_abs = pcall(VimFs.abspath, name)
    ...
    local function _edit_buffer_name(win, newname, bang)
        if newname == win.buffer.name or newname == "" then
            ...
            local newbuf = Buffer(true, false)
            newbuf.name = newname
    ...
    local function _run_map_ex_command(cmd_name, argstr, bang)
        local spec = Commands.get_map_spec(cmd_name)
        ...
        if spec.action == "clear" then
            ...
            Command.clear_mappings(modes, _map_command_opts(opts))
            return true
        end
        ...
        if spec.action == "unmap" then
            ...
            Command.unmap_keys(modes, lhs_seq, _map_command_opts(opts))
            return true
        end
        ...
        if lserr then return lserr end
        local rhs_expanded, rserr = _expand_map_sid(parsed.rhs)
        if rserr then return rserr end
        ...
        if spec.recursive then
            Command.remap_keys(modes, lhs_seq, rhs_seq, map_opts)
        else
            Command.noremap_keys(modes, lhs_seq, rhs_seq, map_opts)
        end
        return true
    end
    ...
    local function _menu_item_matches(name, item, pat)
        ...
        return translated and _menu_path_matches(translated, pat) or false
    end
    ...
    local function _run_menu_ex_command(cmd_name, argstr, bang, parsed_args)
        ...
        local raw = strip(argstr)
        local args = parsed_args or split_ws(raw)
        local idx = 1
        local menu_opts
    ...
    local function command_args(argstr, cmdctx)
        if cmdctx and type(cmdctx.args) == "table" then
            return cmdctx.args
        end
        if argstr == nil or argstr == "" then
            return EMPTY_ARGS
        end
        return split_ws(argstr)
    end
    ...
    function rt:_invoke_builtin(cmd, argstr, bang, cmdctx)
        ...
        if Commands.get_menu_spec(cmd) then
            local rv = _run_menu_ex_command(cmd, argstr, bang, cmdctx and cmdctx.args)
            if Error.IsError(rv) then error(rv) end
            return rv
        end
        ...
        elseif cmd == "source" then
            local ok, err = ScriptSource.source(strip_trailing_comment(argstr))
            if not ok then error(err) end
            return true
        ...
        elseif cmd == "sign" then
            ...
            local function echo_placed(placed)
                for i = 1, #placed do
                    local item = placed[i]
                    ExMsg.echo(("--- Signs for buffer=%s ---"):format(tostring(item.bufnr)))
            ...
            if sub == "define" then
                ...
                local rv = Builtins.fn.sign_define(name, dict)
                if rv ~= 0 then
                    error(Error(474, argstr))
                end
                return true
            elseif sub == "undefine" then
                local rv = Builtins.fn.sign_undefine(tokens[2])
                if rv ~= 0 and rv ~= nil then
                    error(Error(474, argstr))
                end
                return true
            elseif sub == "list" then
                local defs = Builtins.fn.sign_getdefined(tokens[2])
                echo_defs(defs)
                return true
            elseif sub == "place" then
                ...
                if first_num and not first_is_kv then
                    ...
                    if kv.name then
                        ...
                        local rv = Builtins.fn.sign_place(
                            first_num,
                            kv.group or "",
                            kv.name,
                            resolve_buf_arg(kv),
                ...
                local placed = Builtins.fn.sign_getplaced(resolve_buf_arg(kv), dict)
                echo_placed(placed)
                return true
            elseif sub == "unplace" then
                ...
                if not first then
                    ...
                    local placed = Builtins.fn.sign_getplaced(curbuf, { group = "*", lnum = curline })
                    if #placed > 0 and #placed[1].signs > 0 then
                        local top = placed[1].signs[1]
                        Builtins.fn.sign_unplace(top.group, { buffer = curbuf, id = top.id })
                    end
                    return true
                end
                ...
                if next(opts) == nil then opts = nil end
                local rv = Builtins.fn.sign_unplace(group, opts)
                if rv == -1 then
                    error(Error(474, argstr))
                end
                return true
            elseif sub == "jump" then
                ...
                local kv = parse_kv(3)
                local lnum = Builtins.fn.sign_jump(id, kv.group or "", resolve_buf_arg(kv))
                if lnum == -1 then
                    error(Error(474, argstr))
                end
                return true
        ...
        elseif cmd == "tcd" then
            ...
            return true
        elseif cmd == "lua" then
            local ok, rv = LuaLoader.Eval(argstr)
            if not ok then
                error(rv)
            end
            return rv
        ...
        elseif cmd == "redir" then
            ...
            local ok_end, end_err = ExMsg.EndRedir()
            if not ok_end then
                error(end_err)
            end
            ...
            if spec.kind == "file" then
                ...
                local abspath = VimFs.abspath(path)
        ...
        elseif cmd == "runtime" then
            ...
            for _, base in ipairs(rtp) do
                for i = 1, #args do
                    local matches = Filesystem.ExpandWildcards(base .. "/" .. args[i])
                    if #matches ~= 0 then
                        for _, m in ipairs(matches) do
                            if fs.isDir(m) then
                                ExMsg.echo("Cannot source a directory: " .. m)
                            else
                                local ok, err = ScriptSource.source(m)
                                if not ok then
        ...
        elseif cmd == "colorscheme" then
            ...
            if name == "" then
                ExMsg.echo(_current_colorscheme_name())
                return true
            end
        ...
        elseif cmd == "packadd" then
            local name = strip(argstr)
            if name == "" then error(Error(471)) end
            local ok, err = Pack.add(name, { no_load = bang })
            if not ok then error(err) end
            return true
        ...
        elseif cmd == "edit" then
            local win = windows[curwin]
            local newname = (#command_args(argstr, cmdctx) == 0) and "" or argstr
        ...
        elseif cmd == "file" then
            ...
            if target == "" and not remove_name then
                ExMsg.echo(_file_status_message(win, bang))
                return true
            end
            local oldname = win.buffer.name
            if oldname ~= target then
                if oldname ~= "" then
                    local oldbuf = Buffer(false, false)
                    oldbuf.name = oldname
        ...
        elseif cmd == "delete" then
            ...
            if post_mode == "print" then
                ExMsg.echo(_delete_print_text(buf:get_line(target_line, true) or "") .. "\n")
            elseif post_mode == "list" then
                ExMsg.echo(_delete_list_text(buf:get_line(target_line, true) or "") .. "\n")
            end
            win:mark_redraw()
            return true
        ...
        elseif cmd == "drop" then
            ...
            local target_abs = VimFs.abspath(target_raw)
            ...
            if Error.IsError(ok) then
                ...
                newbuf:Load(true)
                local newwin = Window(newbuf, curwin_obj)
                if not _split_real(split_opts.target_winnr, newwin, split_opts.vertical, split_opts.place_after) then
                    error(Error(36))
                end
                enterWindow(newwin.winnr)
        ...
        elseif cmd == "split" then
            ...
            if #command_args(argstr, cmdctx) ~= 0 then
                targetbuf = Buffer(true, false)
        ...
        elseif cmd == "syntax" then
            ...
            if not head or head == "" then
                return Syntax.ExecuteCommand(win, raw)
            end
            local h = head:lower()
            if h == "on" or h == "enable" then
                local ok, err = ScriptSource.source_runtime("syntax/syntax.vim")
                if not ok then error(err) end
                return true
            elseif h == "off" then
                local ok, err = ScriptSource.source_runtime("syntax/nosyntax.vim")
                if not ok then error(err) end
                return true
            elseif h == "manual" then
                local ok, err = ScriptSource.source_runtime("syntax/manual.vim")
                if not ok then error(err) end
                return true
            elseif h == "reset" then
                local ok, err = ScriptSource.source_runtime("syntax/syntax.vim")
        ...
        elseif cmd == "syntime" then
            ...
            if head == "" or head == "report" then
                ...
                for i = 1, #lines do ExMsg.echo(lines[i]) end
                return true
            elseif head == "on" then
                Syntax.SyntimeSet(win, true)
                return true
            elseif head == "off" then
                Syntax.SyntimeSet(win, false)
                return true
        ...
        elseif cmd == "match" then
            ...
            if raw ~= "" and raw:lower() ~= "none" then
                ...
                if not Highlight.HasGroup(group) then
                    error(Error(474, argstr))
                end
            end

            local ok, emsg = Syntax.MatchCommand(win, slot, argstr)
            if not ok then
        ...
        elseif cmd == "highlight" then
            ...
            local changed = false
            if #args == 0 then
                _emit_highlight_listing(Highlight.ListNames())
                return true
            end
            if args[1] == "clear" then
                Highlight.Clear(args[2])
                changed = true
            elseif args[2] == "NONE" then
                Highlight.Clear(args[1])
                changed = true
            elseif #args == 1 and Highlight.HasGroup(args[1]) then
                ...
                if args[1] == "link" then
                    ...
                    if is_default and Highlight.HasGroup(from) then return true end
                    local rv = Highlight.Link(from, to, nil, bang)
                    if Error.IsError(rv) then
                    ...
                    for k, v in pairs(params) do
                        ...
                        elseif k == "guibg" and v:match("^#%x%x%x%x%x%x$") then
                            Highlight.SetGroupColor(args[1], "bg", { rgb = tonumber("0x" .. v:sub(2)) })
                            changed = true
                        elseif k == "gui" or k == "cterm" then
                            ...
                            Highlight.SetGroupReverse(args[1], reverse)
                            changed = true
                        end
                    end
                end
...
function Runtime.PrepareApiState()
