...
assert_eq("targeted redraw does not mark global windows", what_redraw.windows, nil)
...
assert_eq("tabline redraw marks tabline only", what_redraw.tabline, true)
assert_eq("tabline redraw does not mark windows", what_redraw.windows, nil)

reset_flags()
api.nvim__redraw({ statusline = true, win = win1.winnr, flush = false })
assert_eq("statusline win redraw marks target window", win1.need_redraw, true)
assert_eq("statusline win redraw leaves other window", win2.need_redraw, false)
assert_eq("statusline win redraw does not mark global windows", what_redraw.windows, nil)

reset_flags()
api.nvim__redraw({ statusline = true, flush = false })
assert_eq("statusline global redraw marks first window", win1.need_redraw, true)
assert_eq("statusline global redraw marks second window", win2.need_redraw, true)
assert_eq("statusline global redraw does not mark windows flag", what_redraw.windows, nil)
