...
local impl = dofile('vim/lib/luaapi/lpeg.lua')
...
local function assert_same(name, fn)
    ...
    if a.ok ~= b.ok then
        error(("%s: success mismatch (ref=%s impl=%s)"):format(name, tostring(a.ok), tostring(b.ok)))
    end

    if a.ok then
        if not deep_equal(a.vals, b.vals) then
            error(("%s: return mismatch"):format(name))
        end
        return
    end

    if a.err ~= b.err then
        error(("%s: error mismatch\nref: %s\nimpl: %s"):format(name, a.err, b.err))
    end
end

assert(impl.version() == '1.1.0', 'version mismatch')
assert(impl.type(impl.P('x')) == 'pattern', 'type() should identify patterns')
assert(impl.type('x') == nil, 'type() should reject non-pattern')

local parity_cases = {
    {
        name = 'literal match',
        fn = function(M)
            return M.match(M.P('abc'), 'abcdef')
        end,
    },
    {
        name = 'literal anchored fail',
        fn = function(M)
            return M.match(M.P('bc'), 'abc')
        end,
    },
    {
        name = 'init negative index',
        fn = function(M)
            return M.match(M.P('c'), 'abc', -1)
        end,
    },
    {
        name = 'boolean true pattern',
        fn = function(M)
            return M.match(M.P(true), 'abc')
        end,
    },
    {
        name = 'boolean false pattern',
        fn = function(M)
            return M.match(M.P(false), 'abc')
        end,
    },
    {
        name = 'set match',
    ...
    },
    {
        ...
        fn = function(M)
            return M.match(M.R('az') ^ 1 * M.P(-1), 'hello')
        end,
    },
    {
        name = 'ordered choice',
        fn = function(M)
            return M.match((M.P('ab') + M.P('a')) * M.P(-1), 'ab')
        end,
    },
    {
        name = 'sequence',
        fn = function(M)
            return M.match(M.P('a') * M.P('b') * M.P(-1), 'ab')
        end,
    },
    {
        name = 'subtraction success',
        fn = function(M)
            return M.match(M.P('ab') - M.P('c'), 'ab')
        end,
    },
    {
        name = 'subtraction fail',
        fn = function(M)
            return M.match(M.P('ab') - M.P('a'), 'ab')
        end,
    },
    {
        name = 'negative predicate',
        fn = function(M)
            return M.match(-M.P('a') * M.P(1), 'b')
        end,
    },
    {
        name = 'and predicate',
        fn = function(M)
            return M.match(#M.P('a') * M.P('a'), 'a')
        end,
    },
    {
        name = 'repetition possessive',
        fn = function(M)
            return M.match((M.P(1) ^ 0) * M.P('b'), 'ab')
        end,
    },
    {
        name = 'optional repetition',
        fn = function(M)
            return M.match((M.P('a') ^ -1) * M.P(-1), 'a')
        end,
    },
    {
        name = 'bounded repetition',
        fn = function(M)
            return M.match((M.P('a') ^ -2) * M.P(-1), 'aa')
        end,
    },
    {
        name = 'pow fractional error',
        fn = function(M)
            return (M.P('a') ^ 1.5):match('a')
        end,
    },
    {
        name = 'lookbehind',
        fn = function(M)
            return M.match(M.B('a') * M.P('b'), 'ab', 2)
        end,
    },
    {
        name = 'grammar recursion named start',
        fn = function(M)
            return M.match(M.P({ 'S', S = M.P('a') * M.V('S') + M.P('') }), 'aaa')
        end,
    },
    {
        name = 'grammar recursion numeric start',
        fn = function(M)
            return M.match(M.P({ M.P('a') * M.V(1) + M.P('') }), 'aaa')
        end,
    },
    {
        name = 'grammar missing initial rule error',
        fn = function(M)
            return M.match(M.C({ 'missing' }), 'x')
        end,
    },
    {
        name = 'simple capture with nested captures',
        fn = function(M)
            return M.match(M.C(M.P('ab') * M.Cc(9)), 'ab')
        end,
    },
    {
        name = 'constant capture nil slot',
        fn = function(M)
            return M.match(M.Cc(nil, 'x'), '')
        end,
    },
    {
        name = 'anonymous group no inner capture',
        fn = function(M)
            return M.match(M.Cg(M.P('ab')), 'ab')
        end,
    },
    {
        name = 'anonymous group with inner captures',
        fn = function(M)
            return M.match(M.Cg(M.Cc(1) * M.Cc(2)), '')
        end,
    },
    {
        name = 'named group and backref with whole match',
        fn = function(M)
            return M.match(M.Cg(M.P('ab'), 'k') * M.Cb('k'), 'ab')
        end,
    },
    {
        name = 'position capture',
        fn = function(M)
            return M.match(M.Cp() * M.P('a'), 'a')
        end,
    },
    {
        name = 'argument capture nil value',
        fn = function(M)
            return M.match(M.Carg(2), '', 1, nil)
        end,
    },
    {
        name = 'argument capture missing error',
        fn = function(M)
            return M.match(M.Carg(3), '', 1, nil)
        end,
    },
    {
        name = 'backref missing error',
        fn = function(M)
            return M.match(M.Cb('missing'), '')
        end,
    },
    {
        name = 'fold capture',
    ...
    {
        name = 'division number no capture error',
        fn = function(M)
            return M.match(M.P('ab') / 2, 'ab')
        end,
    },
    {
        name = 'division string capture index error',
        fn = function(M)
            return M.match(M.P('ab') / '[%1]', 'ab')
        end,
    },
    {
        name = 'division string %0',
        fn = function(M)
            return M.match(M.P('ab') / '<%0>', 'ab')
        end,
    },
    {
...
for i = 1, #parity_cases do
    local case = parity_cases[i]
    assert_same(case.name, case.fn)
end

print('lpeg parity tests: OK')
