...
let g:finish_seen = 2
    ...
    assert_true("finish avoids invoke_command", code:find("runtime:invoke_command(\"finish\"", 1, true) == nil)
end
...
let g:nmap_bar_split = 1
    ...
    assert_true("nmap invoke generated", code:find("runtime:invoke_command(\"nmap\", \"<buffer> <silent> <nowait> - <Plug>NetrwBrowseUpDir\", false)", 1, true) ~= nil)
...
let g:windo_bar_split = 1
    ...
    assert_true("windo keeps full command chain", code:find("runtime:invoke_command(\"windo\", \"if getline(2) =~# \\\"Netrw\\\" | let s:netrwcnt= s:netrwcnt + 1 | endif\", false)", 1, true) ~= nil)
...
nnoremap <silent><buffer> [[ m':call search('^\s*\(fu\%[nction]\\|def\)\>', "bW")<CR>
    ...
    assert_true("mapping compile emits nnoremap invoke", code:find("runtime:invoke_command(\"nnoremap\", \"<silent><buffer> [[ ", 1, true) ~= nil)
    assert_true("mapping compile keeps escaped regex alternation text", code:find("\\\\|def", 1, true) ~= nil)
    assert_true("mapping compile does not create bogus def command", code:find("runtime:invoke_command(\"def\",", 1, true) == nil)
end
