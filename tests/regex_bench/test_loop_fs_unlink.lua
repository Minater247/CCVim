...
local ok, err, errname = Loop.fs_unlink("/file.txt")
assert_eq("sync unlink ok", ok, true)
assert_eq("sync unlink err nil", err, nil)
assert_eq("sync unlink errname nil", errname, nil)
assert_eq("sync unlink removed path", exists("/file.txt"), false)

local miss_ok, miss_err, miss_name = Loop.fs_unlink("/missing.txt")
...
local dir_ok, dir_err, dir_name = Loop.fs_unlink("/dir")
assert_eq("sync dir ok nil", dir_ok, nil)
assert_true("sync dir err text", type(dir_err) == "string" and dir_err ~= "", dir_err)
assert_eq("sync dir errname", dir_name, "EISDIR")

local ro_ok, ro_err, ro_name = Loop.fs_unlink("/readonly.txt")
...
local req = Loop.fs_unlink("/async.txt", function(cb_err, success)
    async_called = true
    async_err = cb_err
...
assert_eq("async removed path", exists("/async.txt"), false)

local async_miss_called, async_miss_err, async_miss_success = false, nil, "unset"
Loop.fs_unlink("/missing-again.txt", function(cb_err, success)
