...
fun! s:NetrwTrace(msg)
  if exists("g:netrw_trace") && g:netrw_trace
    unsilent echomsg "[netrw-trace] ".a:msg
  endif
endfun
...
fun! s:NetrwBrowse(islocal,dirname)
  call s:NetrwTrace("NetrwBrowse enter islocal=".a:islocal." dirname=".string(a:dirname)." ft=".&ft." buf=".bufnr("%")." name=".bufname("%"))
  ...
  if exists("s:netrw_skipbrowse")
    ...
    call s:NetrwTrace("NetrwBrowse skipbrowse set; return")
    return
  endif
  if !exists("*shellescape")
    call s:NetrwTrace("NetrwBrowse missing shellescape(); return")
    NetrwKeepj call netrw#ErrorMsg(s:ERROR,"netrw can't run -- your vim is missing shellescape()",69)
    return
  endif
  if !exists("*fnameescape")
    call s:NetrwTrace("NetrwBrowse missing fnameescape(); return")
  ...
  call s:NetrwOptionsSave("w:")
  call s:NetrwTrace("NetrwBrowse options saved dirname=".string(dirname))
  ...
  let b:netrw_curdir= dirname
  call s:NetrwTrace("NetrwBrowse set b:netrw_curdir raw=".string(b:netrw_curdir)." dirname=".string(dirname))
  if b:netrw_curdir =~ '[/\\]$'
  ...
  call s:NetrwTrace("NetrwBrowse set b:netrw_curdir final=".string(b:netrw_curdir))
  ...
  " -------------------------------
  call s:NetrwTrace("NetrwBrowse perform listing start dirname=".string(dirname))
  NetrwKeepj call s:NetrwMaps(a:islocal)
  NetrwKeepj call s:NetrwCommands(a:islocal)
  NetrwKeepj call s:PerformListing(a:islocal)
  call s:NetrwTrace("NetrwBrowse perform listing done ft=".&ft." line$=".line("$"))
  ...
  call s:NetrwTrace("NetrwBrowse return ft=".&ft." buf=".bufnr("%")." name=".bufname("%"))
  return
endfun
...
fun! s:NetrwFile(fname)
  ...
  let l:netrw_branch = "unset"
  let l:netrw_is_unix_abs = strpart(fname,0,1) == '/'
  ...
  call s:NetrwTrace("NetrwFile fname=".string(a:fname)." cleaned=".string(fname)." first=".string(strpart(fname,0,1))." is_unix_abs=".l:netrw_is_unix_abs." keepdir=".g:netrw_keepdir." branch=".l:netrw_branch." bcurdir=".string(exists("b:netrw_curdir") ? b:netrw_curdir : "<missing>")." ret=".string(ret))
...
fun! s:NetrwGetWord()
  ...
  let dirname_before_strip= dirname
  ...
  call s:NetrwTrace("NetrwGetWord line=".line(".")." style=".w:netrw_liststyle." raw=".string(dirname_before_strip)." stripped=".string(dirname))
...
fun! s:NetrwBrowseChgDir(islocal,newdir,cursor,...)
  let ykeep= @@
  call s:NetrwTrace("NetrwBrowseChgDir enter islocal=".a:islocal." cursor=".a:cursor." a:newdir=".string(a:newdir)." b:netrw_curdir=".string(exists("b:netrw_curdir") ? b:netrw_curdir : "<missing>"))
  if !exists("b:netrw_curdir")
  ...
  if newdir !~ dirpat && !(a:islocal && isdirectory(s:NetrwFile(s:ComposePath(dirname,newdir))))
    ...
    call s:NetrwTrace("NetrwBrowseChgDir file-branch dirname=".string(dirname)." newdir=".string(newdir)." marker=".((dirname =~ '[*@=|]\r\=$')? '1' : '0'))
    ...
    if a:0 < 1
      ...
      if a:islocal
        ...
        call s:NetrwTrace("NetrwBrowseChgDir pre-edit dirname=".string(dirname)." marker=".((dirname =~ '[*@=|]\r\=$')? '1' : '0'))
  ...
  call s:NetrwTrace("NetrwBrowseChgDir return dirname=".string(dirname)." b:netrw_curdir=".string(exists("b:netrw_curdir") ? b:netrw_curdir : "<missing>"))
  return dirname
endfun
...
fun! s:NetrwEditFile(cmd,opt,fname)
  ...
  call s:NetrwTrace("NetrwEditFile enter cmd=".a:cmd." opt=".string(a:opt)." fname=".string(a:fname)." marker=".((a:fname =~ '[*@=|]\r\=$')? '1' : '0'))
  if exists("g:netrw_altfile") && g:netrw_altfile && &ft == "netrw"
    "   call Decho("exe NetrwKeepj keepalt ".a:opt." ".a:cmd." ".fnameescape(a:fname))
    call s:NetrwTrace("NetrwEditFile exec keepalt ".a:cmd." ".string(fnameescape(a:fname)))
    ...
    call s:NetrwTrace("NetrwEditFile exec ".a:cmd." ".string(fnameescape(a:fname)))
    exe "NetrwKeepj ".a:opt." ".a:cmd." ".fnameescape(a:fname)
  endif
  call s:NetrwTrace("NetrwEditFile post buf=".bufname("%"))
...
fun! netrw#LocalBrowseCheck(dirname)
  ...
  call s:NetrwTrace("LocalBrowseCheck enter dirname=".string(a:dirname)." ft=".&ft." buf=".bufnr("%")." name=".bufname("%"))
  let l:netrw_probe_path = s:NetrwFile(a:dirname)
  let l:netrw_probe_isdir = isdirectory(l:netrw_probe_path)
  let l:netrw_probe_bcurdir = exists("b:netrw_curdir") ? b:netrw_curdir : "<missing>"
  call s:NetrwTrace("LocalBrowseCheck probe dirname=".string(a:dirname)." path=".string(l:netrw_probe_path)." isdir=".l:netrw_probe_isdir." cwd=".getcwd()." bcurdir=".string(l:netrw_probe_bcurdir))
  let ykeep= @@
  if l:netrw_probe_isdir
    ...
    if &ft != "netrw" || (exists("b:netrw_curdir") && b:netrw_curdir != a:dirname) || g:netrw_fastbrowse <= 1
      ...
      call s:NetrwTrace("LocalBrowseCheck case1 invoke NetrwBrowse dirname=".string(a:dirname))
      sil! NetrwKeepj keepalt call s:NetrwBrowse(1,a:dirname)

    elseif &ft == "netrw" && line("$") == 1
      "    call Decho("case 2 (ft≡netrw && line($)≡1)",'~'.expand("<slnum>"))
      call s:NetrwTrace("LocalBrowseCheck case2 invoke NetrwBrowse dirname=".string(a:dirname))
      sil! NetrwKeepj keepalt call s:NetrwBrowse(1,a:dirname)

    elseif exists("s:treeforceredraw")
      "    call Decho("case 3 (treeforceredraw)",'~'.expand("<slnum>"))
      unlet s:treeforceredraw
      call s:NetrwTrace("LocalBrowseCheck case3 invoke NetrwBrowse dirname=".string(a:dirname))
      sil! NetrwKeepj keepalt call s:NetrwBrowse(1,a:dirname)
    endif
    call s:NetrwTrace("LocalBrowseCheck postbrowse ft=".&ft." line$=".line("$")." buf=".bufnr("%")." name=".bufname("%"))
    "   call Decho("tab#".tabpagenr()." win#".winnr()." buf#".bufnr("%")."<".bufname("%")."> line#".line(".")." col#".col(".")." winline#".winline()." wincol#".wincol(),'~'.expand("<slnum>"))
  ...
  let @@= ykeep
  call s:NetrwTrace("LocalBrowseCheck non-directory dirname=".string(a:dirname))
  "  call Decho("settings buf#".bufnr("%")."<".bufname("%").">: ".((&l:ma == 0)? "no" : "")."ma ".((&l:mod == 0)? "no" : "")."mod ".((&l:bl == 0)? "no" : "")."bl ".((&l:ro == 0)? "no" : "")."ro fo=".&l:fo,'~'.expand("<slnum>"))
  "  call Decho("tab#".tabpagenr()." win#".winnr()." buf#".bufnr("%")."<".bufname("%")."> line#".line(".")." col#".col(".")." winline#".winline()." wincol#".wincol(),'~'.expand("<slnum>"))
...
fun! s:NetrwLocalListingList(dirname,setmaxfilenamelen)
  ...
  let liststar   = s:NetrwGlob(dirname,"*",0)
  let listdotstar= s:NetrwGlob(dirname,".*",0)
  let filelist   = liststar + listdotstar
  call s:NetrwTrace("NetrwLocalListingList start dirname=".string(dirname)." bcurdir=".string(exists("b:netrw_curdir") ? b:netrw_curdir : "<missing>")." cwd=".getcwd()." dirnamelen=".dirnamelen)
  call s:NetrwTrace("NetrwLocalListingList glob lens star=".len(liststar)." dot=".len(listdotstar)." total=".len(filelist)." sample0=".string(get(filelist,0,""))." sample1=".string(get(filelist,1,""))." sample2=".string(get(filelist,2,"")))
  ...
  let resultfilelist = []
  for filename in filelist
    let l:netrw_fileabs = s:NetrwFile(filename)
    let l:netrw_ftype = getftype(filename)
    let l:netrw_isdir = isdirectory(l:netrw_fileabs)
    call s:NetrwTrace("NetrwLocalListingList entry filename=".string(filename)." fileabs=".string(l:netrw_fileabs)." ftype=".string(l:netrw_ftype)." isdir=".l:netrw_isdir)

    if l:netrw_ftype == "link"
      " indicate a symbolic link
      let pfile= filename."@"

    elseif l:netrw_ftype == "socket"
      " indicate a socket
      let pfile= filename."="

    elseif l:netrw_ftype == "fifo"
      " indicate a fifo
      let pfile= filename."|"

    elseif l:netrw_isdir
      " indicate a directory
      let pfile= filename."/"

    elseif exists("b:netrw_curdir") && b:netrw_curdir !~ '^.*://' && !l:netrw_isdir
      if has("win32")
    ...
    call s:NetrwTrace("NetrwLocalListingList pre-strip pfile=".string(pfile)." dirname=".string(dirname)." dirnamelen=".dirnamelen)
    let pfile= strpart(pfile,dirnamelen)
    let pfile= substitute(pfile,'^[/\\]','','e')
    call s:NetrwTrace("NetrwLocalListingList post-strip pfile=".string(pfile))
